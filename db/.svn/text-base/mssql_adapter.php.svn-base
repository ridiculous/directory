...
class MssqlAdapter {
    ...
    public function _ddp() {
        return "CASE WHEN p.program_id > 0 THEN ( ISNULL(d.dep_acronym,'') + ' - '+ ISNULL(r.pro_name,'')) ELSE ISNULL(d.dep_acronym,'') END";
    }

    public function _ddp_join_id() {
        return "CASE WHEN p.program_id > 0 THEN r.department_id ELSE p.department_id END";
    }
